...
export default function MarketTab({ sector, setSector, onAnalyze, loading }) {
  ...
  return (
    <div>
      ...
      {loading && (
        <div style={{ textAlign: 'center', color: '#818cf8', padding: '12px 0', fontSize: 12, marginBottom: 10 }}>
          📡 正在載入真實股價資料...
        </div>
      )}
        ...
        {sec.stocks.map(stock => {
          ...
          return (
            ...
            >
              ...
              <button onClick={() => onAnalyze(stock, Object.entries(SECTORS).find(([, s]) => s.stocks.find(st => st.code === stock.code))?.[0])} style={{
                width: '100%', padding: '7px', fontSize: 12,
                borderRadius: 7, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 700,
                background: sec.color + '18', border: `1px solid ${sec.color}44`, color: sec.color,
              }}>
                🧠 AI 分析
              </button>
            </div>
