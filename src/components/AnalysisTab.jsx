...
export default function AnalysisTab({ stock, sectorKey, allSectors, analysis, busy, onReanalyze }) {
    ...
    return (
      <div style={{ textAlign: 'center', color: '#475569', padding: '70px 0' }}>
        ...
        <div style={{ fontSize: 12, marginTop: 8, color: '#374151' }}>
          本地 K 線引擎即時分析，或使用 Claude AI 深度分析
        </div>
      </div>
    )
  }
  ...
  const sigColor = s =>
    s === '買入' ? '#22c55e' : s === '賣出' ? '#ef4444' : '#94a3b8'
  const sigBg = s =>
    s === '買入' ? '#0f2b1e' : s === '賣出' ? '#2b0f0f' : '#1e2d4d'
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10, flexWrap: 'wrap' }}>
        <div>
          ...
          <span style={{ fontSize: 12, color: '#475569', marginLeft: 8 }}>{stock.code} · {sec?.name}</span>
        </div>
        ...
        {analysis && !analysis.error && (
          <span style={{
            padding: '3px 10px', borderRadius: 20, fontSize: 12, fontWeight: 800, marginLeft: 'auto',
      ...
      <div style={{ display: 'flex', gap: 12, fontSize: 11, marginBottom: 8, flexWrap: 'wrap' }}>
        {[
          { label: 'MA5',  val: ma5,  colored: true },
          { label: 'MA10', val: ma10, colored: true },
          { label: 'MA20', val: ma20, colored: true },
        ].map(({ label, val, colored }) => (
          <span key={label} style={{
            color: colored && val ? (last.close > val ? '#22c55e' : '#ef4444') : '#94a3b8',
        ...
        {rsi14 !== null && (
          <span style={{ color: rsi14 > 70 ? '#ef4444' : rsi14 < 30 ? '#22c55e' : '#94a3b8' }}>
            <span style={{ color: '#475569' }}>RSI </span>
            {rsi14.toFixed(1)}
      ...
      {busy && (
        <div style={{ textAlign: 'center', color: '#818cf8', padding: '24px 0', fontSize: 13 }}>
          ⏳ 分析 K 線型態中...
        </div>
      )}
        ...
        <div>
              ...
              {[
                ...
                <div key={i} style={{ background: '#090d1b', border: '1px solid #1e2d4d', borderRadius: 8, padding: '10px 12px' }}>
                  ...
                  <div style={{ fontSize: 16, fontWeight: 900, color: x.c, fontVariantNumeric: 'tabular-nums' }}>
                    {typeof x.v === 'number' ? fmtPrice(x.v) : x.v || '—'}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ background: '#090d1b', border: '1px solid #1e2d4d', borderRadius: 8, padding: 14 }}>
            <div style={{ fontSize: 10, color: '#475569', marginBottom: 8 }}>
              {analysis.isLocal ? '📐 本地技術分析（免費）' : '🤖 Claude AI 深度分析'}
            </div>
            ...
            {!analysis.error && (
              <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 11, color: '#94a3b8' }}>{analysis.confidence}%</span>
                </div>
              </>
            )}
          </div>
