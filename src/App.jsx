...
import {
  ...
  fetchRealCandles, generateCandles, calcMA, calcRSI,
...
export default function App() {
  ...
  const { customSectors, customStockInfo, addSector, deleteSector, addStock, removeStock } = useCustomStocks()
    ...
    ...Object.fromEntries(
      customSectors.map(s => [s.id, {
        ...
        stocks: s.stocks.map(code => ({
          code, name: customStockInfo[code]?.name || code,
          base: customStockInfo[code]?.base || 100,
          otc: customStockInfo[code]?.otc,
        })),
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <button onClick={manualRefresh} title="刷新股價" style={{
      ...
      <div style={{ padding: '14px 16px', overflowY: 'auto', maxHeight: 'calc(100vh - 140px)' }}>
        ...
        {tab === 'analysis' && (
          <AnalysisTab
            stock={selStock} sectorKey={selSector} allSectors={allSectors}
      ...
      {showAdd  && (
        <AddStockModal
          ...
          onAddStock={(sid, s) => {
            addStock(sid, s)
            setLoadedSectors(p => { const n = {...p}; delete n[sid]; return n })
            pop(`✅ ${s.name || s.code} 已加入`, 'buy')
          }}
